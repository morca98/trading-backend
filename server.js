...
function generateSignal(candles, price, macroTrend, trend15m, atr, liqData, symbol, dailyTrend) {
  ...
  if (isStrongBull && adx > minADX && rsi < 65 && (macroTrend === 'UP' || macroTrend.includes('BULL'))) {
    // Aplicar filtro diário: só long se EMA15 > EMA40 diário
    ...
    // Aplicar filtro diário: só short se EMA15 < EMA40 diário
...
app.get('/api/signal', async function(req, res) {
  try {
    ...
    var results = await Promise.all([
      ...
      axios.get(BINANCE + '/api/v3/klines?symbol=' + symbol + '&interval=1d&limit=60') // Adicionar candles diários
    ]);
    ...
    var ema15Daily = calcEMA(closesDaily.slice(-15), 15);
    var ema40Daily = calcEMA(closesDaily.slice(-40), 40);
    var dailyTrend = ema15Daily > ema40Daily ? 'BULL' : 'BEAR';
