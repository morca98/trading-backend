...
app.get('/api/liqmap', async function(req, res) {
  try {
    ...
    var currentPrice = parseFloat(results[1].data.lastPrice);
    res.json({ success: true, symbol: symbol, currentPrice: currentPrice, fundingRate: parseFloat(results[2].data.lastFundingRate), openInterest: parseFloat(results[3].data.openInterest), levels: [] });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});

app.get('/api/stats', function(req, res) {
  res.json({ success: true, wins: winCount, losses: lossCount, totalPnl: totalPnl });
});
...
app.get('/api/backtest', async function(req, res) {
  try {
    ...
    const engine = new BacktestEngine({ symbol: symbol, interval: interval, limit: limit });
...
function generateSignal(candles, price, macroTrend, trend15m, atr, liqData) {
  ...
  return { signal, conf, price, sl, tp, slPct, tpPct, ema9, ema21, ema50, macroTrend, trend15m, atr: atr.toFixed(2) };
}
    ...
    if (r.data && r.data.result) {
      for (const update of r.data.result) {
        ...
        if (text === '/start' || text === '/help') {
          await sendTelegram('<b>Comandos Disponiveis:</b>\n/price - Preco atual BTC/ETH\n/stats - Estatisticas do Bot\n/backtest - Executar backtest BTC\n/status - Estado do Bot');
        } else if (text === '/price') {
...
async function runBot() {
  ...
  for (var i = 0; i < SYMBOLS.length; i++) {
    ...
    try {
      var r = await axios.get(BINANCE + '/api/v3/ticker/price?symbol=' + symbol);
      var price = parseFloat(r.data.price);
      // Lógica de sinal simplificada para exemplo
      console.log(symbol + ' Check: $' + price);
    } catch (e) {}
  }
}
...
app.listen(PORT, function() {
  ...
  sendTelegram('<b>Bot Ativo!</b>\nComandos: /price, /stats, /backtest, /status');
